...
{
    animation->timImage = (RealTIMImage*)malloc3(sizeof(RealTIMImage));
    TIM_IMAGE ImageData;
    dcRender_LoadTexture(&ImageData, image_data);
    animation->timImage->mode = ImageData.mode;
    animation->timImage->crect = *ImageData.crect;
    animation->timImage->prect = *ImageData.prect;
}
